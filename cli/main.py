...
from orchestration import ReviewPipeline, AgentCoordinator
from testing import TestRunner, CodeValidator
...
Examples:
  ...
  python -m cli.main test --base main
    ...
    parser.add_argument(
        "--version", action="version", version="agent-to-agent 0.1.0"
    )
    ...
    review_parser = subparsers.add_parser(
        "review", help="Run AI-powered code review"
    )
    review_parser.add_argument(
        "--files", "-f", nargs="+", help="Specific files to review"
    )
    review_parser.add_argument(
    ...
    review_parser.add_argument(
        "--diff", help="Git diff range (e.g., HEAD~3..HEAD)"
    )
    review_parser.add_argument(
    ...
    review_parser.add_argument(
        "--include-codebase", action="store_true",
        help="Include relevant codebase context in review"
    )
    review_parser.add_argument(
        "--cross-validate", action="store_true",
        help="Enable cross-validation between agents"
    )
    
    # Validate command
    validate_parser = subparsers.add_parser(
        "validate", help="Validate files without AI review"
    )
    validate_parser.add_argument(
        "--files", "-f", nargs="+", required=True, help="Files to validate"
    )
    validate_parser.add_argument(
        "--repo", "-r", help="Path to repository"
    )
    validate_parser.add_argument(
    ...
    test_parser = subparsers.add_parser(
        "test", help="Run tests on files"
    )
    test_parser.add_argument(
        "--files", "-f", nargs="+", help="Specific files to test"
    )
    test_parser.add_argument(
        "--base", "-b", default="main", help="Base branch for changed files"
    )
    test_parser.add_argument(
        "--repo", "-r", help="Path to repository"
    )
...
async def run_review(args) -> int:
    ...
    # Collect context
    context = git.collect_context(args.base, args.head)
    ...
    if args.cross_validate:
        ...
        result = await coordinator.coordinate_review(files, context, cross_validate=True)
...
def generate_markdown_report(result) -> str:
    ...
    for severity in ["critical", "high", "medium", "low", "info"]:
        severity_findings = [f for f in result.consolidated_findings 
