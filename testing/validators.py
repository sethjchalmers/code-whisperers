...
class CodeValidator:
    ...
    def validate_python(
        ...
        try:
            ...
            status = ValidationStatus.VALID if not any(i.severity == "error" for i in issues) else ValidationStatus.WARNING
    ...
    def validate_terraform(
        ...
        secret_patterns = [
            (r'(password|secret|api_key|access_key)\s*=\s*"[^"]+"', "Potential hardcoded credential"),
        ]
        ...
        status = ValidationStatus.VALID if not any(i.severity == "error" for i in issues) else ValidationStatus.INVALID
