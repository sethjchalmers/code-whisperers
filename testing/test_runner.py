...
class TestRunner:
    ...
    async def run_all_tests(
        self, files: dict[str, str], test_types: list[str] | None = None
    ...
    async def run_python_tests(
        self, files: dict[str, str], test_path: str | None = None
    ) -> TestSuiteResult:
        ...
        result = await self._run_command([python_exe, "-m", "pytest", "--version"])
    ...
    async def run_terraform_validation(self, files: dict[str, str]) -> TestSuiteResult:
        ...
        for tf_dir in tf_dirs:
            ...
            init_result = await self._run_command(
                ["terraform", "init", "-backend=false"], cwd=str(dir_path)
            )
            ...
            validate_result = await self._run_command(
                ["terraform", "validate", "-json"], cwd=str(dir_path)
            )
            ...
            fmt_result = await self._run_command(
                ["terraform", "fmt", "-check", "-diff"], cwd=str(dir_path)
            )
            ...
            suite.results.append(
                TestResult(
                    ...
                    error_message=(
                        fmt_result.error_message if fmt_result.status != TestStatus.PASSED else None
                    ),
                )
            )
    ...
    async def run_kubernetes_validation(self, files: dict[str, str]) -> TestSuiteResult:
        ...
        for file_path, content in files.items():
            ...
            if has_kubeval:
                ...
                try:
                    result = await self._run_command(["kubeval", "--strict", temp_path])
    ...
    async def run_jenkins_lint(self, files: dict[str, str]) -> TestSuiteResult:
        """Lint Jenkins pipeline files."""
        suite = TestSuiteResult(suite_name="jenkins", started_at=datetime.now())

        jenkins_files = [
            f for f in files.keys() if "jenkinsfile" in f.lower() or f.endswith(".groovy")
        ]
        ...
        if not has_lint:
            ...
            for file_path in jenkins_files:
                ...
                if full_path.exists():
                    result = await self._run_command(["npm-groovy-lint", "--files", str(full_path)])
    ...
    async def _run_command(self, args: list[str], cwd: str | None = None) -> TestResult:
    ...
    async def run_custom_test(
        self, command: list[str], name: str, cwd: str | None = None
    ) -> TestResult:
